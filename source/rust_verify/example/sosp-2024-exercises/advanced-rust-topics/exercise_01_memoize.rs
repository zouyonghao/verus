...
impl RwLockPredicate<HashMapWithView<Args, Output>> for Pred {
    open spec fn inv(self, v: HashMapWithView<Args, Output>) -> bool {
        // EXERCISE: fill me in
        forall |k:Args| v@.contains_key(k) ==> #[trigger auto]
            v@[k] == func(&k)
    }
}
...
impl Memoizer {
        ...
        ensures out == func(args)
    {
        ...
        let m = read_handle.borrow();
        if m.contains_key(args) {
            // assert(m.get(args).unwrap() == func(args));
            m.get(args).unwrap().clone()
        } else {
            let out = expensive_function(args);
            let (mut m, write_handle) = self.lock.acquire_write();
            m.insert(args.clone(), out);
            write_handle.release_write(m);
            assert(out == func(args));
            out
        }
    }
}
