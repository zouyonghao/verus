...
mod doubly_linked_list {
    ...
    impl<V> DoublyLinkedList<V> {
                ...
                self@ =~= old(self)@.push(v),
        {
            ...
            let (node, Tracked(mut points_to_node)) = PPtr::<Node<V>>::new(
            ...
            proof {
                ...
                self.ghost_state.borrow_mut().points_to_map.tracked_insert(0, points_to_node);
                ...
                *v == self@[i as int]
        {
            ...
            while j < i
                invariant
                    0 <= j <= i
                    && current_ptr == self.ptr_at(j as int)
            {
                proof {
                    assert(self.well_formed_node(j as int)); // trigger
                }
                let tracked current_pointsto = self.ghost_state.borrow().points_to_map.tracked_borrow(j as int);
                let current_node = current_ptr.borrow(Tracked(current_pointsto));
